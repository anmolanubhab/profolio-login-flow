...
export function ProfileSummaryCard({ activeTab = 'feed', hasCompany = false }: ProfileSummaryCardProps) {
  ...
  const groups: { label: string; items: NavItem[] }[] = [
    {
      label: '',
      items: [
        { label: 'Social Feed', icon: Home, onClick: () => navigate('/dashboard'), isActive: onDashboard && activeTab === 'feed' },
      ],
    },
    {
      label: 'My Activity',
  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-card overflow-hidden">
      ...
      {/* One cohesive navigation panel: a light divider + small caption
          between logical groups, consistent row height/icon size/hover
          state throughout, no per-group card boundaries. */}
      <nav className="border-t py-2">
        {groups.map((group, i) => (
          <div key={group.label || i} className={cn(i > 0 && 'mt-1 pt-1 border-t border-border/60')}>
            {group.label && (
              <div className="px-4 pt-1.5 pb-0.5 text-[10.5px] font-semibold uppercase tracking-wide text-muted-foreground/80">
                {group.label}
              </div>
            )}
