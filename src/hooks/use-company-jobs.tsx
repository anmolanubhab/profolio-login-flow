...
export function useCompanyJobs(companyId?: string) {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      // Fetch application stats for these jobs
      const jobsWithStats = await Promise.all((data || []).map(async (job) => {
...
export function useUserApplications() {
  ...
  const withdrawApplication = async (applicationId: string) => {
    try {
      ...
      if (error) throw error;
      // Update local state to reflect withdrawn status instead of removing it
