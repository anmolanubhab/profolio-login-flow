-- =====================================================================
-- Phase K3-C Remediation — R4 provider readiness (DB portion)
--
--  * _ad_currency_minor_units(text): currency-aware minor-unit divisor so
--    zero-decimal currencies (JPY/KRW/…) are handled correctly instead of
--    assuming 100 everywhere.
--  * ad_billing_apply_webhook: understand two more provider-neutral events
--      - payment_intent.processing  -> async "still working" signal
--      - charge.dispute.created     -> chargeback opened; hold the account
--
-- No Stripe, no production provider, no real money. active_provider stays
-- 'simulated' and test_mode stays true (ad_provider_config guard trigger
-- makes that immutable without a migration).
-- =====================================================================

-- ---- currency minor units ------------------------------------------
create or replace function public._ad_currency_minor_units(_currency text)
returns integer
language sql
immutable
...
as $$
  select case upper(coalesce(_currency, ''))
    when 'JPY' then 1 when 'KRW' then 1 when 'VND' then 1 when 'CLP' then 1
    when 'XOF' then 1 when 'XAF' then 1 when 'BIF' then 1 when 'DJF' then 1
    when 'GNF' then 1 when 'ISK' then 1 when 'KMF' then 1 when 'PYG' then 1
    when 'RWF' then 1 when 'UGX' then 1 when 'VUV' then 1 when 'XPF' then 1
    else 100
  end
$$;

comment on function public._ad_currency_minor_units(text) is
  'Minor units per major unit for a currency (1 for zero-decimal currencies, else 100). Immutable helper for display/formatting.';

-- ---- webhook processor: add processing + dispute events -----------
create or replace function public.ad_billing_apply_webhook(
  _provider public.ad_payment_provider, _provider_event_id text, _event_type text,
  _signature_valid boolean, _payload jsonb
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if _event_type in ('payment_intent.succeeded','payment_intent.payment_failed','payment_intent.requires_action',
                     'payment_intent.canceled','payment_intent.processing','charge.refunded','charge.dispute.created')
  ...
  if _txn.id is not null and _txn.status in ('succeeded','refunded','partially_refunded','canceled')
     and _event_type in ('payment_intent.succeeded','payment_intent.payment_failed','payment_intent.requires_action',
                         'payment_intent.canceled','payment_intent.processing') then
  ...
  elsif _event_type = 'payment_intent.processing' then
    update public.ad_billing_transactions
       set status = (case when status = 'pending' then 'processing' else status end)::public.ad_billing_txn_status,
           provider_ref = coalesce(_payload->>'intent_ref', provider_ref), updated_at = now()
     where id = _txn.id;
    insert into public.ad_billing_events (ad_account_id, event_type, actor_user_id, summary, metadata)
    values (_acct, 'payment_started', null, 'Payment is processing (asynchronous settlement pending)',
  ...
  elsif _event_type = 'charge.dispute.created' then
    -- a chargeback was opened. Do not move the ledger yet (the funds
    -- reversal arrives as its own event / manual adjustment) but stop
    -- delivery immediately by holding the account.
    update public.ad_billing_transactions
       set status = (case when status = 'succeeded' then 'succeeded' else status end)::public.ad_billing_txn_status,
           updated_at = now()
     where id = _txn.id;
    perform public._ad_billing_set_hold(_acct, true, 'payment disputed (chargeback opened)');
    update public.ad_account_billing_state set last_charge_status = 'failed' where ad_account_id = _acct;
    insert into public.ad_billing_events (ad_account_id, event_type, actor_user_id, summary, metadata)
    values (_acct, 'account_hold', null, 'Chargeback / dispute opened — delivery held pending resolution',
            jsonb_build_object('transaction_id', _txn.id, 'dispute_ref', _payload->>'dispute_ref',
                               'reason', _payload->>'dispute_reason'));
