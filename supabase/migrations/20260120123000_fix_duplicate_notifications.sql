...
BEGIN
  ...
  -- Use array_cat to combine, but we will deduplicate next
  ...
  -- This ensures that if the owner is also an admin, they only appear once
  IF v_recipient_ids IS NOT NULL THEN
    SELECT ARRAY(SELECT DISTINCT unnest(v_recipient_ids)) INTO v_recipient_ids;
  END IF;
