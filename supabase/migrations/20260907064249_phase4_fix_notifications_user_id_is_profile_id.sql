-- Bugfix: notifications.user_id references profiles(id), not auth.users(id)
-- (confirmed via pg_constraint just before this migration). Use
-- v_match.candidate_profile_id (already returned by calculate_job_match)
-- instead of the auth user id. No other logic changed.
