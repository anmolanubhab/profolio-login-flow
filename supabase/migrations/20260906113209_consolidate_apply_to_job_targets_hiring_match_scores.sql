-- Only change from the previous version: the match-score snapshot lookup
-- now reads the canonical hiring_match_scores (column `score`, numeric)
-- instead of the retired job_match_scores (column `overall_score`).
-- Every other branch is byte-identical to the definition captured
-- immediately before this migration: resume snapshot handling, duplicate-
-- application ON CONFLICT handling, hiring_application_events logging,
-- owner/admin notification fan-out, authentication/profile checks.
...
begin
  ...
  -- Match-score snapshot: prefer an already-cached hiring_match_scores row
  -- (the canonical table); fall back to computing one now. Never accepts a
  -- client-provided score. Wrapped so a matching-engine error can never
  -- block the application itself.
  begin
    select score into v_match_score
    from public.hiring_match_scores
    where job_id = p_job_id and candidate_user_id = auth.uid();

    if v_match_score is null then
      select hms.score into v_match_score
      from public.calculate_job_match(p_job_id, auth.uid()) hms;
    end if;
