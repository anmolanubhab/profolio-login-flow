-- Phase 4: extract the pure scoring logic (byte-identical to the current
-- calculate_job_match body, minus the auth-check block) into an internal-
-- only helper, so it can be reused by BOTH the client-facing
-- calculate_job_match() (auth-checked) and the trusted scheduled batch
-- processor (process_pending_new_job_matches, added in a later migration,
-- which runs as a system/cron context with no auth.uid() at all) --
-- without duplicating the scoring algorithm a second time anywhere.
create or replace function public._calculate_job_match_core(p_job_id uuid, p_candidate_user_id uuid)
...
-- Not directly callable by any client role -- only calculate_job_match()
-- and the trusted batch processor call it, both of which run as the
-- function owner (SECURITY DEFINER) regardless of grants.
revoke all on function public._calculate_job_match_core(uuid, uuid) from public, anon, authenticated;

comment on function public._calculate_job_match_core(uuid, uuid) is 'Internal-only: the actual scoring algorithm, no auth check. Never call directly from client code -- use calculate_job_match() (auth-checked) instead. Shared by calculate_job_match() and the Phase 4 scheduled batch matcher so the algorithm exists in exactly one place.';

-- calculate_job_match() becomes a thin, auth-checked wrapper -- identical
-- external behavior/signature, just delegates the computation.
...
begin
  ...
  return public._calculate_job_match_core(p_job_id, p_candidate_user_id);
end;
$$;
